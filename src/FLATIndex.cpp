...
//		delete payload;
	...
	void FLATIndex::buildIndex(uint64 fp,SpatialObjectStream* input,string indexFileStem)
	{
		footprint = fp;
	...
	void FLATIndex::initialize(SpatialObjectStream* input,string indexFileStem)
	{
...
//			cout << "Links: " << i << " Frequency: " << frequency[i] << "\n";
	...
	void FLATIndex::query(SpatialQuery *q, vector<SpatialObject *> *result) {
		...
		rtreeVisitorFLAT visitor(&(*q), payload);//, inputStem);
	...
	void FLATIndex::loadIndex(string indexFileStem)
	{
		payload = new PayLoad();
		payload->load(indexFileStem);
		string seedfile = indexFileStem + "_index";
		rtreeStorageManager = SpatialIndex::StorageManager::loadDiskStorageManager(seedfile);
		SpatialIndex::id_type indexIdentifier = 1;
