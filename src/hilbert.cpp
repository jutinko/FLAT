...
#include <string.h>
static void
propagateIntBits(unsigned d, unsigned nBytes,
                 char* c, unsigned y, int fold)
{
...
getIEEEBits(unsigned nDims,
            unsigned ignoreMe __attribute__((unused)),
            char const* cP,
            unsigned y)
...
static void
propogateIEEEBits(unsigned d, unsigned nBytes __attribute__((unused)),
                  char* cP, unsigned y, int fold __attribute__((unused)))
{
...
static unsigned
hilbert_box_vtx_work(unsigned nDims, unsigned nBytes,
                     int findMin,
                     unsigned y,
                     char* c1, char* c2,
                     unsigned rotation,
                ...
                int findMin, void* c1, void* c2)
{
    bitmask_t const one = 1;
    bitmask_t bits = one << (nDims-1);
    return hilbert_box_vtx_work(nDims, nBytes, findMin,
                                nBits, (char*)c1, (char*)c2,
                                0, bits, bits, getIntBits);
}
                     ...
                     int findMin, double* c1, double* c2)
{
    ...
    return hilbert_box_vtx_work(nDims, 8, findMin,
                                max+53, (char *)c1, (char *)c2,
                                rotation, bits, index, getIEEEBits);
}
...
*/
static unsigned
hilbert_box_pt_work(unsigned nDims, unsigned nBytes,
                    int findMin,
                    unsigned max, unsigned y,
                    char* c1, char* c2,
                    unsigned rotation,
               ...
               int findMin, void* c1, void* c2)
{
    bitmask_t const one = 1;
    bitmask_t bits = one << (nDims-1);
    return hilbert_box_pt_work(nDims, nBytes, findMin,
                               0, nBits, (char*)c1, (char*)c2,
                               0, bits, bits,
                               getIntBits, propagateIntBits);
}
                    ...
                    int findMin, double* c1, double* c2)
{
    ...
    if (c1Signs != c2Signs)
    {
        ...
        index = 1;
        hilbert_box_pt_work(nDims, 8, findMin,
...
/* having put everything in the same orthant, start */
    ...
    return hilbert_box_pt_work(nDims, 8, findMin,
                  ...
                  int findPrev, void* c1V, void* c2V, void const* ptV)
{
    ...
    unsigned p_y = 0; // If something breaks here was unitinialized warning
    bitmask_t p_separator = 0, p_firstSeparator = 0; // TODO the same (second
    bitmask_t p_cornerdiff = 0, p_reflection = 0; // and everything until
    bitmask_t p_fold1 = 0, p_fold2 = 0, p_valu1 = 0, p_valu2 = 0; // here)
